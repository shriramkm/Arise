...
- (void)resetTopView
{
    ...
    StateViewController *stateVc = (StateViewController *)vc.slidingViewController.underLeftViewController;
    if([self underLeftShowing]){
        ++stateCounter;
        if(stateCounter%2==0){
            if((stateCounter/2)%2==0){
                labelString = @"ON";
                stateVc.stateLabel.text = labelString;
                NSLog(@"OFF %@",stateVc.stateLabel.text);
                vc.alarmState = NO;
                [vc.alarmTimer invalidate];
                [vc.repeatingTimer invalidate];
                vc.repeatingTimer = nil;
                [vc.diffTimer invalidate];
                vc.diffTimer = nil;
                vc.diffLabel.text = @"";
                ...
                labelString = @"OFF";
                stateVc.stateLabel.text = labelString;
                NSLog(@"ON %@",stateVc.stateLabel.text);
                vc.alarmState = YES;
                [vc setAlarmTime];
